...
# count the number of pairs where one range overlaps the other
count = 0
for i in range(len(pairs)):
	if pairs[i][0][0] <= pairs[i][1][0] and pairs[i][0][1] >= pairs[i][1][0]:
		count += 1
	elif pairs[i][1][0] <= pairs[i][0][0] and pairs[i][1][1] >= pairs[i][0][0]:
		count += 1

# print the number of pairs where one range overlaps the other
print(count)