...
minimumSizePossible = requiredSizeForUpdate - freeSpace

# wrong answer: 11734583
# wrong 1680
# answer: 3866390

smallestSize = 0
smallestNode = None
# find the directory with size > minimumSizePossible and smallest size possible
for pre, fill, node in RenderTree(root):
	# if node is directory
	if node.is_leaf == False:
		# if node has total size > minimumSizePossible
		if node.size > minimumSizePossible:
			# if node has smallest size possible
			if smallestSize == 0 or node.size < smallestSize:
				# set smallest size to node's size
				smallestSize = node.size
				# set smallest node to node
				smallestNode = node

# print smallest node's size
